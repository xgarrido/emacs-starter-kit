...
** trac-wiki-mode
Setting =lisp= directory and add it the =load-path=.
#+begin_src emacs-lisp
  (setq starter-kit-src-directory (concat starter-kit-dir "lisp/"))
  (add-to-list 'load-path starter-kit-src-directory)
#+end_src

First we ensure that =trac-wiki.el= file is present in =lisp= directory.
#+begin_src emacs-lisp
